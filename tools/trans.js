...
let Trans = {
  ...
  getData(uid) {
    ...
    if (!ds.uid || (lodash.isEmpty(ds.avatars) && lodash.isEmpty(ds.chars))) return false
    let ret = Data.getData(ds, "uid,name,level,word,face,card,sign,_profile")
    ret.game = "gs"
    let info = ds.info || dst.info
    if (info) {
      if (info?.stats?.fieldExtMap) delete info.stats.fieldExtMap
      ret.info = info
    }
    ret.avatars = {
      ...(dst.avatars || {}),
  ...
  getArtis(artis) {
    ...
    if (lodash.isEmpty(profile)) {
      if (!lodash.isEmpty(mys)) ret.mysArtis = mys
    } else {
      ret.artis = profile
      let hasDiff = false
      let getKey = (d) => {
      ...
      for (let idx = 1; idx <= 5; idx++) {
        if (getKey(profile[idx]) !== getKey(mys[idx])) hasDiff = true
      }
      if (hasDiff) ret.mysArtis = mys
    }
    return ret
  }
}
