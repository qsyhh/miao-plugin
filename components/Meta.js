...
class MetaData {
  ...
  addData(datas, pk = "id") {
    ...
    lodash.forEach(datas, (ds, id) => {
      id = ds[pk] || id
      data[id] = ds
      if (ds.name && ds.name !== id) alias[ds.name] = id
    })
  }
  ...
  getId(txt) {
    ...
    if (alias[txt] || alias2[txt]) return alias[txt] || alias2[txt]

    if (aliasFn) {
      let id = aliasFn(txt)
      if (alias[id] || alias2[id]) return alias[id] || alias2[id]
    }
    return false
  }
...
const Meta = {
  ...
  create(game, type) {
    let key = `${game}.${type}`
    if (!MetaStore[key]) MetaStore[key] = new MetaData(game, type)
