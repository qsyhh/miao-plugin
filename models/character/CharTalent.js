...
const CharTalent = {
  ...
  getAvatarTalent(char, talent, cons, mode) {
    ...
    let addTalent = {
      gs: { a: 3, e: 3, q: 3 },
      sr: { a: 1, e: 2, q: 2, t: 2, me: 1, mt: 1, j: cons > 4 ? 2 : 1 }
    }
    lodash.forEach(addTalent[game], (addNum, key) => {
      ...
      if (lodash.isNumber(ds)) value = ds
      if (mode !== "level") {
        // 基于original计算level
        ...
        if (value > 10 && isGs) {
          ...
          original = value
          if (key === "a" && isGs) level = aPlus ? value + 1 : value
      ...
      if (mode === "level") {
        ...
        level = value
        if (key === "a" && isGs) original = aPlus ? value - 1 : value
