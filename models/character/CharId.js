...
const CharId = {
  getId(ds = "", game = "", elem = "") {
    ...
    if (match) return ret(match.data, match.game, elem)
    if (game !== "sr") {
      // 尝试使用元素起始匹配
  ...
  getRandomId(game = "gs") {
    ...
    let ids = meta.getIds()
    if (game === "gs") ids = lodash.filter(ids, (id) => /^\d+$/.test(id))
    return lodash.sample(ids)
  },

  isTraveler(id) {
    if (id) return [ 10000007, 10000005, 20000000 ].includes(id * 1)
    return false
  },

  isTrailblazer(id) {
    if (id) return [ 8001, 8002, 8003, 8004, 8005, 8006, 8007, 8008 ].includes(id * 1)
    return false
  }
