...
export default class Player extends Base {
  ...
  get faceImgs() {
    let char
    if (this.isGs && this.face) char = Character.get(this.face)

    if (!char) {
      let charId = lodash.keys(this._avatars)[0]
      if (charId) char = Character.get(charId)
    }
    let imgs = char?.imgs || {}
    return {
  ...
  reload() {
    ...
    if (data._ck) this._ck = data._ck
    if (!data.avatars) this.save()
  }
  ...
  hasAvatar(id = "") {
    if (!id) return !lodash.isEmpty(this._avatars)
  ...
  getAvatar(id, create = false) {
    ...
    if (this.isGs) {
      // 兼容处理旅行者的情况
      if (char.isTraveler && !create) id = avatars["10000005"] ? 10000005 : 10000007
    }
  ...
  getAvatarData(ids = "") {
    ...
    if (!ids) {
      ...
      lodash.forEach(ids, (id) => {
        let avatar = this.getAvatar(id)
        if (avatar) ret[id] = avatar.getDetail()
      })
    }
    return ret
  }
  ...
  getProfile(id) {
    let avatar = this.getAvatar(id)
    if (!avatar.isProfile) return false
  ...
  getUpdateTime() {
    let ret = {}
    if (this._profile) ret.profile = MysAvatar.getDate(this._profile)
    if (this._mys) ret.mys = MysAvatar.getDate(this._mys)
    return ret
  }
  ...
  async refresh(cfg) {
    this.save(false)
    try {
      if (cfg.index || cfg.index === 0) await this.refreshMysInfo(cfg.index)
      if (cfg.detail || cfg.detail === 0) await this.refreshMysDetail(cfg.detail)
  ...
  async refreshAndGetAvatarData(cfg) {
    ...
    this.forEachAvatar((avatar) => {
      ...
      if (avatar.game === "sr") ds.aeq = talent?.a?.original + talent?.e?.original + talent?.q?.original + talent?.t?.original || 4
      avatarRet[ds.id] = ds
