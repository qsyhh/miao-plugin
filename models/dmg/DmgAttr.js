...
let DmgAttr = {
  ...
  getAttr({ originalAttr, attr, weapon, char, game = "gs" }) {
    ...
    // 技能属性记录
    lodash.forEach((game === "gs" ? "a,a2,a3,e,q,nightsoul" : "a,a2,a3,e,e2,q,q2,t,me,me2,dot,break").split(","), (key) => {
      ret[key] = ret[key] || {
        pct: 0, // 倍率加成
  ...
  calcAttr({ originalAttr, buffs, meta, artis, params = {}, incAttr = "", reduceAttr = "", talent = "", game = "gs" }) {
    ...
    lodash.forEach(buffs, (buff) => {
      ...
      lodash.forEach(buff.data, (val, key) => {
        ...
        // 技能提高
        let tRet = /^(a|a2|a3|e|q|t|me|dot|break|nightsoul)(Def|Ignore|Dmg|Enemydmg|Plus|Pct|Cpct|Cdmg|Multi)$/.exec(key)
        if (tRet) {
