...
let DmgAttr = {
  ...
  getAttr({ originalAttr, attr, weapon, char, game = "gs" }) {
    ...
    if (originalAttr) ret = lodash.merge({}, originalAttr)
    ...
    lodash.forEach((game === "gs" ? "mastery,recharge,cpct,cdmg,heal,dmg,phy,diffelem" : "speed,recharge,cpct,cdmg,heal,dmg,enemydmg,effPct,effDef,stance").split(","), (key) => {
      ret[key] = AttrItem.create(originalAttr?.[key] || {
    ...
    if (!originalAttr) {
      ...
      if (game === "gs") {
        ...
        ret.fycdmg = 0 // 反应伤害爆伤提升（百分比/受精通加成）
      } else if (game === "sr") {
        ret.sp = char.sp * 1
        // 超击破
        ret.superBreak = {
  ...
  calcAttr({ originalAttr, buffs, meta, artis, params = {}, incAttr = "", reduceAttr = "", talent = "", game = "gs" }) {
    ...
    lodash.forEach(buffs, (buff) => {
      ...
      if (buff.cons) {
        if (ds.cons * 1 < buff.cons * 1) return
      }
      ...
      lodash.forEach(buff.data, (val, key) => {
        ...
        aRet = /^(mastery|cpct|cdmg|heal|recharge|dmg|enemydmg|phy|diffelem|shield|stance)(Plus|Pct|Inc)?$/.exec(key)
        if (aRet) {
          attr[aRet[1]][aRet[2] ? aRet[2].toLowerCase() : "plus"] += val * 1 || 0
          return
        }
        ...
        if (sRet) attr[sRet[1]][sRet[2].toLowerCase()] += val * 1 || 0
      })
      msg.push(title)
    })
