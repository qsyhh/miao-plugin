...
const baseAttr = {
  gs: "atk,def,hp,mastery,recharge,cpct,cdmg,dmg,phy,heal,shield,diffelem".split(","),
...
class AttrData extends Base {
  ...
  init(data) {
    ...
    if (data) this.setAttr(data, true)
  }
  ...
  addAttr(key, val, isBase = false) {
    ...
    if (this.isSr && Format.isElem(key, this.game)) {
      if (Format.sameElem(this.char.elem, key, this.game)) key = "dmg"
    }

    if (baseAttr[this.game].includes(key)) {
      attr[key].plus += val * 1
      if (isBase) base[key] = (base[key] || 0) + val * 1
      return true
    }
    ...
    if (testRet && testRet[1] && testRet[2]) {
      ...
      attr[key][key2] += val * 1
      if (key2 === "base" || isBase) base[key] = (base[key] || 0) + val * 1
      return true
    }
    return false
  }
  ...
  setAttr(data, withBase = false) {
    ...
    lodash.forEach(data, (val, key) => {
      if (this.isSr && Format.isElem(key, this.game)) {
        if (this.char.elem === Format.elem(key, "", this.game)) this.addAttr("dmg", val)
      } else {
  ...
  getAttr() {
    ...
    lodash.forEach(baseAttr[this.game], (key) => {
      ret[key] = this[key]
      if ([ "hp", "atk", "def", "speed" ].includes(key)) ret[`${key}Base`] = this[`${key}Base`]
    })
    ret._calc = true
