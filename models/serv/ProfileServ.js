...
export default class ProfileServ extends Base {
  ...
  // 获取当前面板服务配置
  getCfg(key, def = "") {
    if (!lodash.isUndefined(this.diyCfg[key])) return this.diyCfg[key]
    if (!lodash.isUndefined(this.sysCfg[key])) return this.sysCfg[key]
    return def
  }
  ...
  async getReqParam(uid, game = "gs") {
    ...
    // 获取请求参数
    if (cfg.request) param = await cfg.request.call(this, api)
  ...
  async response(data, req, game = "gs") {
    // 处理返回
    let cfg = this._cfg
    if (cfg.response) return await cfg.response.call(this, data, req, game)
  }

  execFn(fn, args = [], def = false) {
    let { _cfg } = this
    if (_cfg[fn]) return _cfg[fn].apply(this, args)
    return def
  }
