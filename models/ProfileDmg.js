...
export default class ProfileDmg extends Base {
  ...
  static dmgRulePath(name, game = "gs") {
    ...
    for (let ds of dmgFile) {
      ...
      if (ds.test && !ds.test()) continue
      if (fs.existsSync(path)) return { path, createdBy: ds.name }
    }
    return false
  }
  ...
  talent() {
    ...
    let { isSr, isGs } = this
    lodash.forEach((isSr ? "a,a2,e,e1,e2,q,q2,t,t2,me,me2,me2list,mt,mt1,mt2,j,j2" : "a,e,q").split(","), (key) => {
      let level = lodash.isNumber(talentData[key]) ? talentData[key] : (talentData[key]?.level || 1)
      let keyRet = /^(a|e|q|t|me|mt|j)(1|2)(list)?$/.exec(key)
      if (keyRet) {
        let tmpKey = keyRet[1]
  ...
  async getCalcRule() {
    ...
    if ([ 10000005, 10000007, 20000000 ].includes(this.char.id * 1)) ruleName = `旅行者/${this.profile.elem}`
  ...
  async calcData({ enemyLv = 103, mode = "profile", dmgIdx = 0, idxIsInput = false }) {
    if (!this.char || !this.profile) return false
    let { profile } = this
    let { game } = this.char
    ...
    // 用户手动输入伤害序号
    // 从1开始，所以需要 - 1
    if (idxIsInput) dmgIdx = --dmgIdx < 0 ? 0 : dmgIdx

    if (mode === "single") dmgIdx = defDmgIdx > -1 ? defDmgIdx : 0

    lodash.forEach(details, (detail, detailSysIdx) => {
      if (mode === "single") {
        if (defDmgKey) {
          if (detail.dmgKey !== defDmgKey) return true
