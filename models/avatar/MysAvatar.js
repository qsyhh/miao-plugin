...
const MysAvatar = {
  ...
  checkForce(player, force) {
    let e = player?.e
    let mys = e?._mys
    if (!e || !mys || !mys.isSelfCookie) return force

    let ck = mys?.ckInfo?.ck
    if (!ck || player._ck === ck) return force
    return force
  },
  /**
   * 更新米游社角色信息
  ...
  async refreshMysDetail(player, force = 0) {
    ...
    let mys = e?._mys
    if (!mys) return false
    if (!AvatarUtil.needRefresh(player._mys, force, { 0: 60, 1: 2, 2: 0 })) return false
  ...
  async refreshMysInfo(player, force = 0) {
    ...
    if (!mys) return false
    // 不必要更新
    if (!AvatarUtil.needRefresh(player._info, force, { 0: 60, 1: 2, 2: 0 })) return false

    let infoData = await mys.getIndex()
    if (!infoData || !infoData.role) return false
    ...
    if (charData && charData.avatars) {
      ...
      if (lodash.keys(charIds).length > 8) {
        player.forEachAvatar((avatar) => {
          if (!charIds[avatar.id] && !avatar.isProfile) player.delAvatar(avatar.id)
        })
      }
    }
    if (player.hasAvatar()) {
  ...
  setMysInfo(player, infoData) {
    ...
    lodash.forEach(infoData?.stats || [], (num, key) => {
      ...
      if (key !== "spiral_abyss") stats[lodash.camelCase(key)] = num
    })
    if (stats?.fieldExtMap) delete stats.fieldExtMap

    let exploration = {}
    lodash.forEach(infoData?.world_explorations || [], (ds) => {
      let { name } = ds
      if (name === "层岩巨渊") return true
  ...
  getNeedRefreshIds(player, ids, force = 0) {
    ...
    lodash.forEach(ids, (id) => {
      ...
      if (AvatarUtil.needRefresh(avatar._talent, force, needMap)) ret.push(avatar.id)
    })
    return ret
  },
  ...
  async refreshTalent(player, ids, force = 0) {
    let e = player?.e
    let mys = e?._mys
    if (!e || !mys || !mys.isSelfCookie) return false
    ...
    if (needReqIds.length > 0) {
      if (needReqIds.length > 8) e && e.reply("正在获取角色信息，请稍候...")
  ...
  async refreshAvatarTalent(avatar, mys) {
    if (mys && mys.isSelfCookie) {
      ...
      if (game === "gs" && talentRes && talentRes.skill_list) {
        ...
        if (talentList.length < 3) return false
  ...
  getInfo(player) {
    ...
    player.forEachAvatar((avatar) => {
      ...
      if (avatar.star === 5) {
        avatar5Count++
        if (!avatar.char?.isTraveler) goldCount += (avatar.cons || 0) + 1
      }
      let w = avatar.weapon
      if (w && w.star === 5) goldCount += w.affix * 1
    })
