...
export default class Avatar extends Base {
  ...
  get updateTime() {
    let time = this._time
    if (!time) return ""
    if (lodash.isString(time)) return moment(time).format("MM-DD HH:mm")
    if (lodash.isNumber(time)) return moment(new Date(time)).format("MM-DD HH:mm")
    return ""
  }
  ...
  _get(key) {
    if (charKey.includes(key)) return this.char[key]
  }
  ...
  setBasic(ds = {}, source = "") {
    ...
    if (ds.trees) this.setTrees(ds.trees)
  ...
  setWeapon(ds = {}) {
    ...
    if (this.weapon.level < 20) this.weapon.promote = 0
  }
  ...
  setTalent(ds = false, mode = "original", updateTime = "") {
    ...
    if (updateTime) this._talent = now
  }
  ...
  setAvatar(ds, source = "") {
    ...
    if (ds.artis && source !== "mys") this._artis.setArtisData(ds.artis)
    this.calcAttr()
  }
  ...
  setArtis(ds = {}, isMysArtis = false) {
    ...
    if (!this._mysArtis.hasArtis) this.setArtis(ds, true)
  }
  ...
  toJSON() {
    ...
    if (!lodash.isEmpty(artis)) ret.artis = artis
    if (!this.mysArtis.isSameArtis(this.artis)) ret.mysArtis = this.mysArtis.toJSON()
    return {
      ...ret,
