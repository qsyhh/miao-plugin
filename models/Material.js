...
class Material extends Base {
  ...
  static forEach(type = "all", fn, filter = false, game = "gs") {
    if (!lodash.isFunction(filter)) filter = () => true
    lodash.forEach(mMap, (ds, name) => {
      if (type !== "all" && type !== ds.type) return true
      let obj = Material.get(name, game)
      if (filter(obj)) return fn(obj) !== false
    })
  }
  ...
  get abbr() {
    let name = this.name
    if (this.type === "talent") return Data.regRet(/「(.+)」/, name, 1) || name
    if (this.type === "weapon") return name.slice(0, 4)
  ...
  get label() {
    let abbr = this.abbr
    if (this.city) return `${this.city}·${this.abbr}`
    return abbr
  }
