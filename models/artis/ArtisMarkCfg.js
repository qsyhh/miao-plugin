...
const ArtisMarkCfg = {
  getCharArtisCfg(profile) {
    ...
    let def = function(attrWeight) {
      ...
      if (isGs) {
        ...
        if (weight.atk > 0 && weaponCfg[wn]) {
          ...
          if (check(wCfg.attr, wCfg.max || 75, wCfg.plus || 75)) title.push(wCfg.abbr || wn)
        }

        // 不与攻击力挂钩的武器判定
        if (wn === "辰砂之纺锤" && check("def")) title.push("纺锤")

        // 圣遗物判定，如果是绝缘4，将充能权重拉高至沙漏圣遗物当前最高权重齐平
        let maxWeight = Math.max(weight.atk || 0, weight.hp || 0, weight.def || 0, weight.mastery || 0)
        if (artis.is("绝缘4") && check("recharge", maxWeight, 75, false)) title.push("绝缘4")
      }
    ...
    if (charRule) return charRule({ attr, elem, artis, rule, def, weapon, cons: profile.cons })
  },
  ...
  getCfg(profile) {
    ...
    lodash.forEach(attrMap, (attr, key) => {
      ...
      if (!weight || weight * 1 === 0) return true
