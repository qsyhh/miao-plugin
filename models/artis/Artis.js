...
export default class Artis extends Base {
  ...
  _get(key) {
    ...
    if (artis[key]) return artis[key]
  }

  forEach(fn) {
    lodash.forEach(this.artis, (ds, idx) => {
      if (ds.name) return fn(ds, idx)
    })
  }
    ...
    for (let idx = 1; idx <= 5; idx++) {
      ...
      if (ds && (ds.name || ds.id)) {
        ...
        let tmp = {
          ...
          level: ds?.level == 0 ? 0 : ds.level || 1
        }
        if (ds.main && ds.attrs) {
          tmp.main = ds.main || null
          tmp.attrs = []
          for (let attrIdx in ds.attrs || []) {
            if (ds.attrs[attrIdx]) tmp.attrs.push(ArtisMark.formatAttr(ds.attrs[attrIdx]))
          }
        }
        ret[idx] = tmp
      }
  ...
  setArtisBase(idx = 1, ds = {}) {
    ...
    if (this.isSr) {
      ...
      arti.level = arti?.level || ds?.level == 0 ? 0 : ds.level || arti.level || 1
      ...
      arti.level = ds?.level == 0 ? 0 : ds.level || 1
      arti.star = ds.star || 5
    }
  }
  ...
  setArtis(idx = 1, ds = {}) {
    ...
    let arti = this.artis[idx]
    if (!ds.attrIds || !ds.mainId) return false
  ...
  toJSON() {
    ...
    this.eachIdx((ds, idx) => {
      ...
      tmp[key] = ds[key]
      if (this.isGs) tmp.star = ds.star || 5
      ...
      if (!ds.mainId || !ds.attrIds) return true
