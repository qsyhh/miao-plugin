...
class ArtifactSet extends Base {
  ...
  static getByArti(name) {
    ...
    if (arti && arti.set) return ArtifactSet.get(arti.set)
    return false
  }

  static get(name, game = "gs") {
    if (game === "gs" && /^\d{5}$/.test(name)) name = name.toString().slice(0, 2)
    let data = Meta.matchGame(game, "artiSet", name)
    if (data) return new ArtifactSet(data.data, data.game)
    return false
  }

  static getArtiNameBySet(set, idx = 1) {
    let artiSet = ArtifactSet.get(set)
    if (artiSet) return artiSet.getArtiName(idx)
    return ""
  }
