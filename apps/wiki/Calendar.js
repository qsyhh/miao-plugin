...
let Cal = {
          ...
          if (vRet && vRet[1]) {
            ...
            if (content && content[1]) {
              ...
              if (/(将于)(([0-9\\/\\: ]){9,})(进行)/.exec(content)) {
                ...
                if (tRet && tRet[2]) versionTime[vRet[1]] = versionTime[vRet[1]] || tRet[2].replace("06:00", "11:00")
              }
            }
          }
        })
  ...
  getAbyssCal(s1, e1) {
    ...
    lodash.forEach(check, (ds) => {
      let [ s2, e2 ] = ds
      if ((s2 <= s1 && s1 <= e2) || (s2 <= e1 && e1 <= e2)) ret.push(ds)
    })
    return ret
  },
  ...
  getCharData(dateList) {
    ...
    Character.forEach((char) => {
      if (charBirth[char.birth] && (char.isRelease || char.birth !== "1-1")) charBirth[char.birth].push(char.getData("id,name:abbr,star,face"))
   ...
   * @param isAct
   * @param startTime.totalRange
   * @param startTime.now
   * @param startTime.timeMap
   * @returns {boolean}
   */
  getList(ds, target, { startTime, endTime, totalRange, now, timeMap = {} }, isAct = false) {
    ...
    if (fulltimeReg.test(title) || eDate - sDate > 365 * 24 * 3600 * 1000) {
      ...
      if (width > (isAct ? 38 : 55)) label = sDate.format("MM-DD HH:mm") + " ~ " + label
    } else if (sDate > now) {
