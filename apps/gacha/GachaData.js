...
let GachaData = {
  ...
  readJSON(qq, uid, type, game) {
    ...
    lodash.forEach(logJson, (ds) => {
      ...
      if (!id || !itemMap[id] || (ds.id && ids[ds.id])) return true
      ids[ds.id] = true
      items.push({
        id,
        logId: ds.id,
  ...
  analyse(qq, uid, type, game) {
    ...
    let fiveAvg = 0
    let fourAvg = 0
    if (fiveNum > 0) fiveAvg = ((allNum - noFiveNum) / fiveNum).toFixed(2)
    // 平均四星
    if (fourNum > 0) fourAvg = ((allNum - noFourNum) / fourNum).toFixed(2)
  ...
  stat(qq, uid, type, game) {
    ...
    if ([ "all" ].includes(type) && !isSr) loadData(500)
    ...
    if (lodash.isEmpty(items)) return false
    ...
    let getCurr = function() {
      if (currVersion && !lodash.isEmpty(currVersion)) {
        ...
        lodash.forEach(cv.items, (num, id) => {
          ...
          let star = item.star
          if (isUp) temp.upIds[id] = item.name
    ...
    lodash.forEach(items, (ds) => {
      if (!currVersion || (ds.time < currVersion.start && hasVersion)) {
        if (currVersion) versionData.push(getCurr())
        let v = GachaData.getVersion(ds.time, hasVersion, isMix, game)
        if (!hasVersion) v.version = type === "all" ? "全部统计" : "常驻池"
        if (!v) return true
        currVersion = {
          ...v,
          items: {}
