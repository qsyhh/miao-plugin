...
const ProfileStat = {
  ...
  async render(e, mode = "stat") {
    ...
    let starFilter = 0
    if (/(五|四|5|4|)+星/.test(msg)) starFilter = /(五|5)+星/.test(msg) ? 5 : 4
    if (starFilter) avatarRet = lodash.filter(avatarRet, ds => ds.star === starFilter)
    ...
    for (let [ k, v ] of Object.entries(chineseToEnglishElements)) {
      ...
      // e.g. 物(理)?  量(子)?  虚(数)?
      if (msg.includes(k)) elementFilter.push(v)
    }
    if (elementFilter.length > 0) avatarRet = lodash.filter(avatarRet, ds => elementFilter.some(elem => ds.elem.includes(elem)))
