...
let Avatar = {
  async renderAvatar(e, avatar, renderType = "card") {
    ...
    if (typeof (avatar) === "string") {
      ...
      if (!char.isRelease) {
        ...
        if (!avatar) avatar = { id: char.id, name: char.name, detail: false }
      }
    }
  ...
  async renderCard(e, avatar, renderType = "card") {
    ...
    if (!bg) return e.reply(`${char.name}暂无角色图片`)
    if (renderType === "photo") return e.reply(segment.image(`file://${miaoPath}/resources/${bg.img}`))
    ...
    let msgRes = await e.reply([
      await Common.render("character/character-card", {
        saveId: uid,
        uid,
        bg,
